...
DEFINE_MMAP_IMPL(fail) {
	...
	debug("%p == %p, %zu, %zu, %zu, %zu\n",
		addr, HEAP_START, length, test_length, offsetof(struct block_header, contents),
		region_actual_size(test_length + offsetof(struct block_header, contents)));
